...
class TrayIcon:
    ...
    def _try_appindicator(self) -> bool:
        ...
        try:
            ...
            item_show.connect("activate", lambda w: GLib.idle_add(self.app._show_window))
            ...
            item_quit.connect("activate", lambda w: GLib.idle_add(self.app._close_completely))
    ...
    def _try_pystray(self) -> bool:
        ...
        try:
            ...
            self.indicator = pystray.Icon("AI Studio Proxy", image, "AI Studio Proxy", menu)
...
class SimpleGUILauncher:
    ...
    def __init__(self):
        ...
        self.fastapi_port = tk.StringVar(value=str(self.config.get("fastapi_port", 2048)))
        ...
        self.proxy_enabled = tk.BooleanVar(value=self.config.get("proxy_enabled", False))
    ...
    def _create_interface(self):
        ...
        ttk.Label(header_frame, text="🚀 AI Studio Proxy API", 
                 style="Header.TLabel").pack(side=tk.LEFT)
        
        # Status indicator (top right)
        self.status_label = ttk.Label(header_frame, textvariable=self.status,
                                     style="Status.TLabel")
    ...
    def _create_main_tab(self):
        ...
        self.account_combo = ttk.Combobox(row1, textvariable=self.selected_account,
                                        state="readonly", width=30)
        self.account_combo.pack(side=tk.LEFT, padx=(10, 20))
        
        ttk.Label(row1, text="Mode:").pack(side=tk.LEFT)
        ttk.Radiobutton(row1, text="Headless", variable=self.run_mode,
                       value="headless").pack(side=tk.LEFT, padx=(10, 5))
        ttk.Radiobutton(row1, text="Visible", variable=self.run_mode,
                       value="debug").pack(side=tk.LEFT)
        ...
        self.start_btn = tk.Button(btn_frame, text="▶️ START",
                                    ...
                                    font=("Segoe UI", 14, "bold"),
                                    height=2, width=15, cursor="hand2",
        ...
        self.api_url_label = ttk.Label(api_card, 
        ...
        ttk.Button(api_btn_frame, text="🔍 Test",
                  command=self._api_test).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(api_btn_frame, text="🌐 Open in Browser",
                  command=self._open_in_browser).pack(side=tk.LEFT)
    ...
    def _create_account_tab(self):
        ...
        # Account list
        list_frame = ttk.LabelFrame(self.account_tab, text="📋 Saved Accounts", padding="10")
        ...
        ttk.Button(btn_frame, text="➕ Add New Account",
                  command=self._add_new_account).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # Account details
        detail_frame = ttk.LabelFrame(self.account_tab, text="ℹ️ Account Details", padding="10")
        detail_frame.pack(fill=tk.X)
        
        self.account_detail = ttk.Label(detail_frame, 
                                      text="Select an account to see details")
    ...
    def _create_settings_tab(self):
        ...
        # Port settings
        port_frame = ttk.LabelFrame(self.settings_tab, text="🔌 Port Settings", padding="10")
        ...
        ttk.Entry(row1, textvariable=self.fastapi_port, width=10).pack(side=tk.LEFT, padx=(10, 20))
        
        ttk.Label(row1, text="Stream Port:").pack(side=tk.LEFT)
        ttk.Entry(row1, textvariable=self.stream_port, width=10).pack(side=tk.LEFT, padx=(10, 0))
        
        # Proxy settings
        proxy_frame = ttk.LabelFrame(self.settings_tab, text="🌍 Proxy Settings", padding="10")
        proxy_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Checkbutton(proxy_frame, text="Use Proxy", 
                       variable=self.proxy_enabled).pack(anchor=tk.W)
        
        proxy_row = ttk.Frame(proxy_frame)
        proxy_row.pack(fill=tk.X, pady=(5, 0))
        
        ttk.Label(proxy_row, text="Address:").pack(side=tk.LEFT)
        ttk.Entry(proxy_row, textvariable=self.proxy_address, 
                 width=40).pack(side=tk.LEFT, padx=(10, 0))
        
        ttk.Label(proxy_frame, text="Example: http://127.0.0.1:7890",
                 foreground=COLORS["text_secondary"]).pack(anchor=tk.W, pady=(5, 0))
        ...
        ttk.Button(save_frame, text="💾 Save Settings",
                  command=self._save_and_notify).pack(side=tk.LEFT)
        ttk.Button(save_frame, text="🔄 Reset to Default",
                  command=self._reset_config).pack(side=tk.LEFT, padx=(10, 0))
    ...
    def _create_log_tab(self):
        ...
        ttk.Button(btn_frame, text="📂 Open Log File",
                  command=self._open_log_file).pack(side=tk.LEFT)
    ...
    def _delete_account(self):
        ...
        if not messagebox.askyesno("Confirm", f"Are you sure you want to delete '{account_name}'?"):
            return
    ...
    def _api_test(self):
        ...
        try:
            ...
            messagebox.showwarning("Warning", "Could not connect to API.\nIs the service running?")
        except Exception as e:
    ...
    def _reset_config(self):
        ...
        if messagebox.askyesno("Confirm", "All settings will be reset to default. Continue?"):
    ...
    def _start_internal(self, mode: str, account: str = None,
                        save_auth_as: str = None, exit_on_save: bool = False):
        """Internal start function"""
        
        self._log("🔍 Checking ports...")
        if not self._clean_ports():
            if not messagebox.askyesno("Warning",
                "Some ports could not be cleaned. Continue?"):
    ...
    def _read_logs(self):
        ...
        try:
            while self.process and self.process.poll() is None:
                ...
                if line:
                    self.root.after(0, lambda l=line.strip(): self._log(l, save_to_file=False))
